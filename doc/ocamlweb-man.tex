...
\item \textbf{How can I use square brackets in \LaTeX, since they are
  ...
  involving square brackets and put them in a \verb|.tex| file that
  you will provide on the \ocamlweb\ command line. Then, there is no
  longer square brackets in your source comments, but only calls to
  these macros. 

\item \textbf{What about lexers and parsers?} ~\par
  
  This is currently only a pretty-printing support for
  \textsf{ocamllex} lexers and \textsf{ocamlyacc} parsers.  There is
  no indexing for the identifiers defined or used in these modules.
  You have to pass the corresponding files to \ocamlweb\ through the
  option \verb!--impl!.

\item \textbf{I would like my pattern-matching right-hand sides to be
    aligned. Is it possible?} ~\par

  No, it is not, since \ocamlweb\ uses proportional fonts. However you
  can align your pattern-matching in your source files, since
